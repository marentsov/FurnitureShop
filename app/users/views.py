...
class UserLoginView(LoginView):
    ...
    def form_valid(self, form):
        ...
        if user:
            ...
            if session_key:
                # удаляем старую корзину авторизованного пользователя
                forgot_carts = Cart.objects.filter(user=user)
                if forgot_carts.exists():
                    forgot_carts.delete()
                # добавляем новую корзину для авторизованного пользователя из анонимной сессии
...
class UserProfileView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        messages.success(self.request, 'Личная информация обновлена')
        return super().form_valid(form)

    def form_invalid(self, form):
        messages.error(self.request, 'Ошибка обновления личной информации')
        return super().form_invalid(form)

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['title'] = 'Home - кабинет пользователя'
